...
public class RepeatView extends ViewParent {
    ...
    public RepeatView(String Data,String params,String state1,String Return){
        super();
        setData(Data);
        setState1(state1);
        setParams(params);
    ...
    public RepeatView(Context context, AttributeSet attr) {
        super(context, attr);
        init(this.getParams());
    }
    ...
    private void init(String params){
       ...
       if(clickable) scope.watch(this.getState1(), new DataListener<Integer>() {
            @Override
